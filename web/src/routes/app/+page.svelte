		...
		if (!currentUser && clearReason === 'role_deleted') {
			...
			(async () => {
				await dialogs.showConfirmation({
					...
					message:
						'Your judge role has been removed from the event setup. Please select a new role.',
					confirmText: 'OK',
					cancelText: '',
	...
	{#if errorNotices.length > 0}
		<div class="fixed right-4 bottom-4 z-50 space-y-2">
			{#each errorNotices as notice, index (index)}
				<div
					class="flex max-w-sm min-w-xs items-center justify-between rounded-lg bg-red-100 p-4 text-red-700 shadow-lg"
				>
					...
					<button
						...
						class="ml-2 rounded-full p-1 hover:bg-red-200"
