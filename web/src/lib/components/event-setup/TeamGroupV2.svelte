<script lang="ts">
	...
	import { EditingTeamListV2, type TeamInfoAndData } from '$lib/team.svelte';
	...
	interface Props {
		...
		selectedItems: EditingTeamListV2;
		activeZoneId: string;
  ...
  const flipDurationMs = 200;
	...
	async function confirmDeleteGroup() {
		const confirmed = await dialogs.showConfirmation({
			title: 'Delete Group',
			message: `Are you sure you want to delete the group "${groupName}"? All teams in this group will be moved to other groups or become ungrouped.`,
		...
		if (source !== SOURCES.KEYBOARD) {
			if (selectedItems.length && trigger === TRIGGERS.DRAG_STARTED) {
				if (selectedItems.includes(id)) {
					...
					selectedItems = new EditingTeamListV2();
				}
			}
		}

		if (trigger === TRIGGERS.DRAG_STOPPED) selectedItems = new EditingTeamListV2();
		teamList = newItems;
	}
		...
		if (selectedItems.length) {
			if (trigger === TRIGGERS.DROPPED_INTO_ANOTHER) {
				...
				tick().then(() => {
					...
					teamList = newItems;
					activeZoneId = zoneId;
					if (source !== SOURCES.KEYBOARD) selectedItems = new EditingTeamListV2();
				});
			}
		} else {
			teamList = newItems;
...
<div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
	...
	<div class="mb-3 flex items-center justify-between">
		<div class="flex flex-1 items-center gap-2">
			...
			{/if}
			<span class="text-xs text-gray-500">({teamList.length})</span>
		</div>
