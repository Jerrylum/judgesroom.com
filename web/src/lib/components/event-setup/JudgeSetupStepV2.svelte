<script lang="ts">
	import JudgeGroupV2 from './JudgeGroupV2.svelte';
			...
			if (trigger === TRIGGERS.DROPPED_INTO_ANOTHER) {
				...
				tick().then(() => {
					...
					const selectedTeamObjects = selectedItems.map(selectedId => {
						return unassignedTeams.find(t => t.id === selectedId) || 
							   teamsProp.find(t => t.id === selectedId);
	...
	function handleMaybeSelect(teamId: string, e: MouseEvent | KeyboardEvent) {
		...
		if (selectedItems.includes(teamId)) {
			selectedItems = selectedItems.filter(id => id !== teamId);
		} else {
			selectedItems.push(teamId);
		}
	}
	...
	function deleteJudgeGroup(groupId: string) {
		...
		if (index > -1) {
			...
			// Move assigned teams back to unassigned
			const teamInfosToMove = group.assignedTeams.map(editingTeam => {
				// Convert EditingTeam back to TeamInfoAndData
				return {
					id: editingTeam.id,
			...
			unassignedTeams = [...unassignedTeams, ...teamInfosToMove];
	...
	function randomlyAssignTeams() {
		// Convert TeamInfoAndData to EditingTeam for the randomlyAssignTeamsToGroups function
		const editingTeams = teamsProp
			.filter((team) => !team.absent)
			.map(team => {
		...
		randomlyAssignTeamsToGroups(editingTeams, judgeGroups);
			...
			>
				{#each unassignedTeams as team (team.id)}
					<div
						...
						tabindex="0"
					>
						<TeamPlateV2 {team} showGrade={!allTeamsHaveSameGrade} onEditTeam={() => {}} />
					</div>
				{/each}
