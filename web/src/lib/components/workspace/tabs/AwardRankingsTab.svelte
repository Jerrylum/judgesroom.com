	...
	<div class="mx-auto max-w-4xl space-y-6">
		...
		<div class="rounded-lg bg-white p-6 shadow-sm">
			...
			<p class="mt-2 text-sm text-gray-600">
				Rank teams for specific awards based on judging criteria and performance.
			</p>
		</div>

		<!-- Award Selection -->
		<div class="rounded-lg bg-white p-6 shadow-sm">
			<h3 class="text-lg font-medium text-gray-900 mb-4">Award Selection</h3>
			<div>
		...
		{#if selectedAwardId && eligibleTeams().length > 0}
			<div class="rounded-lg bg-white p-6 shadow-sm">
				<h3 class="text-lg font-medium text-gray-900 mb-4">Team Rankings</h3>
				<p class="text-sm text-gray-600 mb-4">
					Drag teams to reorder, or use the arrow buttons to adjust rankings.
				</p>
			...
			<div class="rounded-lg bg-white p-6 shadow-sm">
				<h3 class="text-lg font-medium text-gray-900 mb-4">Ranking Justification</h3>
				<textarea
				...
				<div class="mt-2 text-xs text-gray-500">
					Document the reasoning behind these rankings for transparency and future reference.
				</div>
			</div>

			<!-- Top 3 Summary -->
			<div class="rounded-lg bg-white p-6 shadow-sm">
				<h3 class="text-lg font-medium text-gray-900 mb-4">Top 3 Teams</h3>
					...
					{#each teamRankings.slice(0, 3) as teamId, index (teamId)}
						...
						{#if team}
							<div class="text-center p-4 rounded-lg"
								...
								<div class="text-xs text-gray-500 mt-1">{team.school}</div>
							</div>
						{/if}
					{/each}
				</div>
			...
			<div class="flex justify-end space-x-3">
				<button onclick={saveRankings} class="primary">
					Save Rankings
				</button>
			</div>
		{:else if selectedAwardId && eligibleTeams().length === 0}
			<div class="rounded-lg bg-yellow-50 p-6">
				<div class="text-center text-yellow-800">
					...
					<p class="text-sm mt-1">There are no teams that meet the criteria for this award.</p>
				</div>
			</div>
		{/if}
	</div>
